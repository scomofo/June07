...
from app.core.threading import Worker # Correctly import Worker (QRunnable)
...
JD_DEALER_ACCOUNT_NO_CONFIG_KEY = "JD_DEALER_ACCOUNT_NO" # Assuming this key exists in global_config
...
class RecentDealsView(BaseViewModule):
    ...
    def _view_quote_details(self):
        ...
        # Corrected API client access path
        ...
        if not actual_api_client or not hasattr(actual_api_client, 'get_maintain_quote_details'):
            QMessageBox.critical(self, "API Error", "MaintainQuotesAPI (via JDQuoteIntegrationService) or its get_maintain_quote_details method is not available."); return

        self.status_label.setText(f"Fetching details for Quote ID: {self.current_quote_id_for_dialog}..."); self.set_ui_enabled(False)
        # Use Worker (QRunnable) correctly
    ...
    def _call_jd_api_get_quote_details(self, quote_id: str, dealer_account_no: str, po_number: Optional[str]):
        try:
            ...
            if not actual_api_client or not hasattr(actual_api_client, 'get_maintain_quote_details'):
                ...
                raise AttributeError("MaintainQuotesAPI client or method not found.")

            return actual_api_client.get_maintain_quote_details(quoteId=quote_id, params=params)
    ...
    def _handle_quote_details_response(self, details: Any):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle(f"Quote Details: {self.current_quote_id_for_dialog or 'N/A'}")
        dialog.setMinimumSize(700, 500)

        layout = QVBoxLayout(dialog)
        text_edit = QTextEdit(dialog)
        text_edit.setPlainText(details_str)
