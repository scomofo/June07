...
class MaintainQuotesAPI:
    ...
    async def get_external_quote_status(self, external_quote_id: str, dealer_account_no: Optional[str] = None, po_number: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        Args:
            ...
            dealer_account_no (Optional[str]): The dealer account number for the quote.
            po_number (Optional[str]): The purchase order number for the quote.
        ...
        try:
            response_result: Result[Dict, BRIDealException] = await self.jd_quote_api_client.get_quote_details(
                ...
                dealer_account_no=dealer_account_no, # Pass new arguments
                po_number=po_number # Pass new arguments
            )
...
if __name__ == "__main__":
    ...
    class MockJDQuoteApiClient:
        ...
        async def get_quote_details(self, quote_id: str, dealer_account_no: Optional[str] = None, po_number: Optional[str] = None) -> Result[Dict, BRIDealException]:
            self.logger.info(f"MockJDQuoteApiClient: get_quote_details called for {quote_id}, dealer_account_no: {dealer_account_no}, po_number: {po_number}")
            if not self.is_operational:
                return Result.failure(BRIDealException("Mock API Client not operational."))
            await asyncio.sleep(0.05) # Simulate async operation
            return Result.success({"id": quote_id, "status": "approved", "amount": 5000, "customer": "Mock Customer Inc.", "dealerAccountNo": dealer_account_no, "poNumber": po_number})
    ...
    if maintain_api_ok.is_operational:
        ...
        async def test_get_status_and_quotes():
            if creation_response and creation_response.get("id"):
                status_result = await maintain_api_ok.get_external_quote_status(creation_response.get("id"), dealer_account_no="DA123", po_number="PO456")
